...
mod queries;
use queries::{
    create_dog, create_owner, delete_dog, get_all_owners, get_breeds, get_dog, get_dogs,
    get_dogs_from_owner, get_owner, get_owners_from_dog, search_for, update_dog, update_owner,
};
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let sql_path = app
                .path()
                .resolve("src/init.sql", BaseDirectory::Resource)?;
            let mut full_path = String::new();
            tokio::runtime::Runtime::new()
                .unwrap()
                .block_on(check_database_creation(
                    database_path,
                    sql_path,
                    &dir_path,
                    &mut full_path,
