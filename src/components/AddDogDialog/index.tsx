...
export default function AddDogDialog({ open, onClose }: AddDogDialogProps) {
  ...
  if (ownersQuery.isPending || breedsQuery.isPending)
    return <DialogContent>Carregando...</DialogContent>;

  if (ownersQuery.isError || breedsQuery.isError)
    return <DialogContent>Algo deu errado.</DialogContent>;

  return (
    <StylishDialog
      open={open}
      onClose={onClose}
      className="max-w-[80%] w-full md:max-w-[70%] lg:max-w-[60%]"
    >
        ...
        <div className={styles.wholeDialog}>
          ...
          <div className={`${styles.stackRow} ${styles.fiftyFifty}`}>
            <TextField
              value={dogName}
              name="dog_name"
              onChange={(value) => setDogName(value)}
              variant="outlined"
              label="Nome"
              isRequired
            />
            <Select
              ...
              onSelectionChange={(key) => {
                if (key != '-1') setDogOwner(key?.toString() ?? '');
              }}
              variant="outlined"
              name="dog_owner"
              isRequired
          ...
          <div className={`${styles.stackRow} ${styles.sixtyForty}`}>
            <Select
              ...
              label="Raça"
              disabledKeys={'0'}
              onSelectionChange={(key) => setDogBreed(key?.toString() ?? '0')}
            >
            ...
            <Select
              label="Sexo"
              disabledKeys={'0'}
              onSelectionChange={(key) => setDogGender(key?.toString() ?? '0')}
              variant="outlined"
              name="dog_gender"
              isRequired
            >
          ...
          <RadioGroup
            name="isFromAPackage"
            aria-label="Faz parte de um pacote?"
          ...
          {dogClub === 'yes' && <TextField label="Valor" variant="outlined" />}
        </div>
