...
export default function StylishSnack({
  text,
  open,
  onClose,
  ...
  const animTimerRef = React.useRef<NodeJS.Timeout | undefined>();
  ...
  React.useEffect(() => {
    ...
    if (open) {
      ...
      if (closeByItself) {
        timerRef.current = setTimeout(() => onClose && onClose(), 5000);
      }
    } else {
      ...
      animTimerRef.current = setTimeout(() => ref.current?.classList.add('hidden'), 500);
    }
  ...
  }, [open, position, closeByItself, onClose]);

  return (
    <div
      ref={ref}
      className={`hidden opacity-0 ${positionClasses[position].closedTranslate} ${positionClasses[position].positionNames} m-4 fixed z-2 items-center p-4 bg-white rounded-lg
