...
export default function DogDetails() {
  ...
  if (dogObject)
    return (
      <div style={styles.container}>
        <div style={styles.actions}>
          ...
          <DetailsAction
            action={() => setShelveOpen(true)}
            icon={ToFileIcon}
            text={false ? 'Desarquivar' : 'Arquivar'}
          />
          <DetailsAction
            action={() => setEditOn(!editOn)}
            icon={editOn ? CancelIcon : PencilIcon}
            text={editOn ? 'Cancelar' : 'Editar'}
          />
        </div>
        <div>
          ...
          <form action="" style={styles.form}>
            <DetailsField
              editStyle={{ flexGrow: 2 }}
              name={'Nome'}
              value={dogName}
              valueSetter={setDogName}
              placeholder="Insira o novo nome..."
              viewOnly={!editOn}
              whenNothingDisplay="Não informado"
            />
            ...
            <DetailsField
              name={'Raça'}
              value={dogBreed}
              placeholder="Insira a nova raça..."
              type="select"
              viewOnly={!editOn}
              whenNothingDisplay="Não informada"
              viewOnlyDisplay={`${dogBreed?.toString().slice(0, 1)?.toUpperCase()}${dogBreed?.toString().slice(1)}`}
            >
            ...
            <DetailsField
              ...
              viewOnly={!editOn}
              value={dogGender}
              valueSetter={setDogGender}
              viewOnlyDisplay={
                dogGender === 'male' ? 'Macho' : dogGender === 'female' ? 'Fêmea' : undefined
              }
            >
              <MenuItem value="" disabled>
                Nenhum
        ...
        <Dialog sx={{ zIndex: 2 }} open={shelveOpen} onClose={() => setShelveOpen(false)}>
          ...
          <DialogContent>
            <DialogContentText>
              Reconheça que após o arquivamento, é possível achar {dogName} com o filtro de
              busca.
            </DialogContentText>
            <DialogActions>
        ...
        <Fab
          color="primary"
          variant="extended"
          sx={{ display: editOn ? 'block' : 'none', position: 'fixed', right: 30, bottom: 30 }}
          onClick={saveDog}
        >
          Salvar
        </Fab>
    ...
    return (
      <ErrorPage
        title="Erro ao tentar visualizar."
        subtitle="Não foi possível encontrar esse pet."
      />
    );
}
