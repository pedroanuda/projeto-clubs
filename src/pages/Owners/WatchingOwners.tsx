...
export default function WatchingOwners() {
  ...
  React.useEffect(() => {
    if (location.pathname.includes('/view/')) setViewMode(true);
    else setViewMode(false);
    if (location.pathname.includes('/create')) setCreateMode(true);
  ...
  const handleSubmit = React.useCallback(
    (e: React.FormEvent) => {
      ...
      navigate(
        trimmedValue !== '' ? { search: `?search=${trimmedValue}` } : { search: undefined }
      );
    },
    [searchValue]
  );
  ...
  return (
    <>
      <div className="flex h-full text-on-surface">
        <div
          className={`grow-1 flex-col ${viewMode || createMode ? 'hidden md:flex' : 'flex'}`}
        >
          ...
          <SearchBar
            placeholder="Pesquise por donos"
            className="mx-4 mt-2"
            value={searchValue}
        ...
        <section
          ref={detailsRef}
          className={`peer/secou w-full h-full overflow-y-auto box-border
